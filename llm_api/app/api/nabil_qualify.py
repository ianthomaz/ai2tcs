...
def _system_prompt() -> str:
    return (
        ...
        "llm_theme_notes (mandatory style): 1–2 short pt-BR sentences summarizing the SUBSTANCE of the post "
        "(who, what, where, São Paulo urban/political context when relevant). "
        "Direct factual notes only — NOT meta-commentary about reading the caption (avoid the old English pattern "
        "\"The caption discusses…\" and the same in Portuguese). "
        "Do NOT cast the caption/text as the grammatical subject (no \"A legenda…\", \"O texto…\", \"O autor fala sobre…\" "
        "as framing); start with the topic itself (e.g. \"Crítica à privatização de parques na zona X.\"). "
        "FORBIDDEN openings for llm_theme_notes include: \"A legenda discute\", \"A legenda aborda\", "
        "\"A legenda explica\", \"A legenda menciona\", and close variants. If you wrote any of those, rewrite "
        "llm_theme_notes into plain factual content only. Vary wording across posts; do not reuse the same opener. "
        "Variety (mandatory): across different captions, change syntactic shape for llm_theme_notes and summary_140 "
        "(sometimes start with place or actor, sometimes with action or policy object, sometimes with a short "
        "clause in parentheses) — avoid applying one rigid sentence template to every item. "
        "Do not make llm_theme_notes and summary_140 mirror the same opening words or skeleton."
    )
...
def _user_prompt(caption: str, rag_block: str | None, repair_hint: str | None) -> str:
    ...
    parts.append(
        ...
        "Campo llm_theme_notes: resumo curto e directo do tema (quem, o quê, onde, contexto urbano/político de SP "
        "quando couber); proibido começar com \"A legenda discute/aborda/explica/menciona\" ou variações meta "
        "sobre o acto de ler a legenda — se aparecer isso, reescreva só o teor factual em uma ou duas frases curtas, "
        "indo já ao assunto, sem repetir a mesma fórmula de abertura entre posts. "
        "Varie a estrutura das frases entre itens (lugar primeiro, verbo forte, ou tema em destaque); "
        "não use o mesmo esqueleto sintáctico em todas as respostas. summary_140 e llm_theme_notes não devem "
        "copiar a mesma construção inicial."
    )
    return "\n\n".join(parts)
...
async def _ollama_chat(*, model: str, system: str, user: str, num_predict: int) -> str:
    def _call() -> dict:
        return ollama.chat(
            ...
            options={
                "temperature": settings.nabil_qualify_effective_temperature(),
