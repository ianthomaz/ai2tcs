...
async def job_insert_terminal_audit(
    ...
    *,
    ...
    Used for sync HTTP paths (nabil qualify-caption, nfExtract, extract, router, …) so the dashboard / stats see activity.
    """
    try:
        jid = str(uuid4())
        user_context_raw = json.dumps(user_context) if user_context else None
    ...
    async with (await get_pool()).acquire() as conn:
        ...
        avg_row = await conn.fetchrow(
            ...
            WHERE status IN ('done', 'no_answer', 'need_more_info')
              AND COALESCE(job_kind, 'ask') NOT IN (
                'nabil_qualify_caption',
                'nf_extract',
                'extract',
                'extract_multi',
                'router'
