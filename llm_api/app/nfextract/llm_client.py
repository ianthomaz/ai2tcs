...
async def enrich_with_local_llm(
    *,
    ...
    prompt = (
        ...
        "PAYMENT DESTINATION RULES (Bike Anjo / supplier bank account):\n"
        "- Keep payment_* as SEPARATE JSON keys. Never concatenate Pix + bank into one string "
        "and never invent a field named payment_info.\n"
        "- If the document shows BOTH a Pix key and bank account details, fill BOTH sets of "
        "fields (do not drop bank data when Pix exists).\n"
        "- payment_pixCode: the full Pix key (email, phone with DDI, CPF/CNPJ digits, or EVP UUID).\n"
        "- payment_bank: prefer the 3-digit Brazilian COMPE bank code when known "
        "(e.g. \"341\" Itaú, \"237\" Bradesco, \"104\" Caixa, \"001\" Banco do Brasil, "
        "\"033\" Santander, \"077\" Inter, \"260\" Nubank, \"336\" C6). "
        "If only the bank name appears, return the name (e.g. \"Itaú\").\n"
        "- payment_bank_agency: agency digits only (no check digit unless printed as part of agency).\n"
        "- payment_bank_account: account number INCLUDING the check digit when printed "
        "(prefer \"4183659-6\" style with hyphen before the digit).\n"
        "- payment_bank_account_type: only \"Corrente\", \"Poupança\", or null "
        "(never \"unknown\", never free-text paragraphs).\n"
        "- payment_receiver_name / payment_receiver_document: titular/favorecido who RECEIVES "
        "the transfer (may differ from supplier razão social). Digits-only for the document.\n"
        ...
        "CRITICAL: Associação Bike Anjo (CNPJ 19.515.100/0001-89) is almost always the "
        "TOMADOR on supplier NFS-e — never put it in payment_receiver_*. If the document "
        "has no Pix/bank destination for the prestador, leave payment_receiver_* as JSON null. "
        "Do not copy the tomador name/CNPJ into payment_receiver. "
        ...
        "\"dinheiro\" for cash/espécie. When BOTH Pix and bank account exist, still set "
        "payment_type to \"pix\" but keep all bank fields filled.\n"
