#!/usr/bin/env bash
# Setup proxy LLM no bikeanjovm: porta 28472 -> mini62:28471
...
scp -i "$SSH_KEY" "$ROOT/docs/nginx-llm-proxy-bikeanjovm.conf" "$BIKEANJOVM_USER@$BIKEANJOVM_IP:/tmp/llm-proxy.conf"
...
ssh -t -i "$SSH_KEY" "$BIKEANJOVM_USER@$BIKEANJOVM_IP" "
  ...
  echo '[OK] Proxy LLM ativo em bikeanjovm:28472 -> mini62:28471'
