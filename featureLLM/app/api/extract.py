...
async def _extract_payment_nf_confirmation(request: ExtractRequest) -> ExtractResponse:
    ...
    try:
        ...
        return ExtractResponse(extracted=_sanitize_payment_nf_object(parsed))
    except Exception:
        return ExtractResponse(extracted=None)
...
async def extract_multi(request: ExtractMultiRequest, _: None = Depends(require_token)):
    ...
    try:
        response = await asyncio.to_thread(
            ollama.chat,
            model=settings.get_model_name(request.model, default_alias="fast"),
            messages=[
                {"role": "system", "content": system},
                {"role": "user", "content": user_content},
            ],
        ...
        return ExtractMultiResponse(extracted={})
    ...
    for key, val in parsed.items():
        ...
        if norm is not None:
            out[kid] = norm
    return ExtractMultiResponse(extracted=out)


@router.post("/extract", response_model=ExtractResponse)
async def extract(request: ExtractRequest, _: None = Depends(require_token)):
    ...
    if step == "payment_nf_confirmation":
        return await _extract_payment_nf_confirmation(request)
    ...
    try:
        ...
        return ExtractResponse(extracted=value)
    except Exception:
        return ExtractResponse(extracted=None)
