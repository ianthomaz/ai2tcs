...
async def login_submit(request: Request, username: str = Form(...), password: str = Form(...)):
    if not settings.dashboard_user or not settings.dashboard_password:
        return templates.TemplateResponse(request, "login.html", {"error": "Dashboard não configurado"})
    ...
    response.set_cookie("dashboard_session", session_id, httponly=True, max_age=86400)
    return response
