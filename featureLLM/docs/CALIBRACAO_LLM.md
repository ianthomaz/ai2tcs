# Calibração da LLM — desvios e dificuldades

Documento de referência sobre os desvios que a LLM tende a cometer, o que fizemos para mitigar e como manter respostas úteis sem comprometer a API. Faz parte do repositório **ai2tcs** (`featureLLM/docs/`).
...
- Não passa por ingest; é carregado em tempo de request. Ver MANUAL_INTEGRACAO.md § 10 e § 13 (estudosmobi — fluxosLLM).
...
| MANUAL_INTEGRACAO.md § 10 | Otimização de qualidade e instruções por projeto. |
| MANUAL_INTEGRACAO.md § 13 | fluxosLLM e estudosmobi. |
