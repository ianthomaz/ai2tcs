# Manual de Integração — LLM API Local (Tailscale)
...
- **Eixo educacional (`/edu/*`):** chat tutor síncrono (`/edu/chat` valida JSON, **1 retry** se o schema falhar, depois **fallback estruturado fixo** — `reply_structured` preenchido em HTTP 200), exercícios, vocabulário, gramática e progresso no Postgres (mandarim / HSK por defeito). Ver § 3.8 e [EDU_API_CONTRACT.md](./EDU_API_CONTRACT.md). Migração: `20260403000000_edu_tables`.
- **Autenticação em dois modos:** token global `LLM_API_TOKEN` **ou** chave por projeto (`itcs_<project_id>_<hex>`), gerada no **Dashboard → projeto → Chaves API**. Migração e detalhes: [`MANUAL_REINTEGRACAO.md`](MANUAL_REINTEGRACAO.md). Migração DB: `20260408000000_api_keys_and_shared_libraries` (e job `model_alias` em `20260408010000_add_job_model_alias`).
...
- **`POST /ingest/upload`:** upload multipart de ficheiro para pasta do projeto (ou biblioteca partilhada); ver § 3 e [`MANUAL_REINTEGRACAO.md`](MANUAL_REINTEGRACAO.md) §4.
...
**Migração:** ver [`MANUAL_REINTEGRACAO.md`](MANUAL_REINTEGRACAO.md). Podes manter só o token global até migrares cada projeto à mão.
...
| **POST** | **`/nfExtract`** | **Extração de nota fiscal (itcsNFextract)** — corpo **somente** `multipart/form-data`; exatamente **um** campo: `file` (upload) **ou** `server_file_path` **ou** `document_url` (nomes fixos). Manual: [ManualNF_Extract](../../docs/ManualNF_Extract) §3.1. |
...
| **POST** | **`/ingest/upload`** | **Upload multipart** de um ficheiro para o disco do projeto (ou biblioteca partilhada) + fila de ingest incremental — ver [`MANUAL_REINTEGRACAO.md`](MANUAL_REINTEGRACAO.md) §4 |
...
**Contrato e exemplos JSON completos:** [EDU_API_CONTRACT.md](./EDU_API_CONTRACT.md).

| Rota | Função resumida |
|------|------------------|
| `POST /edu/chat` | Mensagem do aluno + `level` + `language` + `history` opcional → resposta estruturada (`reply_structured`, `full_reply_text`); validação + 1 retry + fallback fixo; ver [EDU_API_CONTRACT.md](./EDU_API_CONTRACT.md). |
...
Detalhes de campos e normalização: [EDU_API_CONTRACT.md](./EDU_API_CONTRACT.md).
...
0. **Base de dados:** com código actual, aplicar migrações Prisma/SQL em `featureLLM/prisma/migrations/` (inclui tabela de chaves por projeto). Ver [DEVELOPER_GUIDE.md](./DEVELOPER_GUIDE.md).
...
2. **No ambiente da featureLLM** (onde a API roda), configurar no `.env` a variável de sources do projeto (nome em maiúsculas + `_SOURCES`). Ex. para projeto `meu_projeto`:
   ```bash
...
6. **(Opcional) Chave por projeto:** no **Dashboard → Projetos → [slug] → Chaves API**, gerar uma chave e configurar o cliente com `Authorization: Bearer itcs_...` em vez do token global (ver § 2 e [MANUAL_REINTEGRACAO.md](./MANUAL_REINTEGRACAO.md)).

7. **(Opcional) Bibliotecas partilhadas:** no `config_json` do projeto, `shared_libraries: ["slug-outro-indice"]` — ver [MANUAL_REINTEGRACAO.md](./MANUAL_REINTEGRACAO.md) §5.
...
2. **Variável de ambiente:** configure `MEU_PROJETO_SOURCES=/caminho/biblioteca,/caminho/fluxos` no `.env` da featureLLM — ao chamar ingest sem o projeto existir, a API usa essa variável, cria o projeto e indexa.
...
2. **Script no servidor da API:** se o projeto não tiver como chamar a API (ex.: está em outra rede), alguém com acesso ao servidor onde a featureLLM roda pode executar:
   ```bash
   cd /caminho/para/featureLLM
...
No `.env` da featureLLM, defina **WEBPLACECC_SOURCES** com as duas pastas (caminho principal do projeto + subpastas de conteúdo e fluxos):
...
A qualidade de cada **request** (chamado) e da resposta final depende da calibração fina do projeto. Além das regras automáticas descritas em [CALIBRACAO_LLM.md](./CALIBRACAO_LLM.md), você pode configurar comportamentos específicos no `config_json` do projeto.
...
| Respostas com frases meta ou autodesvalorizantes ("Com base no contexto...", "não encontrei informação") | Ver [CALIBRACAO_LLM.md](./CALIBRACAO_LLM.md); reforçar instruções no projeto (`instrucoes-llm.md`) e conferir prompt base |
...
**Extração de NF (itcsNFextract):** `POST /nfExtract` na mesma API — manual em [ManualNF_Extract](../../docs/ManualNF_Extract) (mesmas duas URL bases).
