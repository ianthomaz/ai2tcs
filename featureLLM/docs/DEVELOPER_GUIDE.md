# Guia do desenvolvedor — LLM API

Como **estender, contribuir e fazer manutenção** da API LLM local. Código e esta doc estão no repositório GitHub **ai2tcs** (`featureLLM/`).

**Para usar a API** (endpoints, exemplos, integração de projetos): use o [MANUAL_INTEGRACAO.md](./MANUAL_INTEGRACAO.md). Este guia é só para quem mexe no código da API.
...
# Ver ../README.md (pasta featureLLM) e ./MANUAL_INTEGRACAO.md nesta pasta docs/ para setup completo.
...
│   ├── run.sh                    # Iniciar server
│   └── setup.sh                  # Setup inicial
│
├── docs/                         # Documentação
│   ├── MANUAL_INTEGRACAO.md      # Como usar (clientes)
│   ├── DEVELOPER_GUIDE.md        # Este arquivo
│   └── OPERACAO_TAILSCALE.md     # Operação remota
│
...
Ao alterar o **system prompt** (`app/rag/prompt.py`), o **worker** (`app/jobs/worker.py`) ou as instruções por projeto, consulte [CALIBRACAO_LLM.md](./CALIBRACAO_LLM.md). Esse documento descreve os desvios conhecidos, mitigações implementadas e boas práticas.
...
4. **Atualize CALIBRACAO_LLM.md** (§ 7) com a nova regra, motivo e resultado esperado.
