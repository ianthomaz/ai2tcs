...
- **Config opcional por arquivo:** `featureLLM/projects/<project_id>/config.json` sobrescreve ou complementa (paths, chunking, policies).
...
- [ ] Documentar como expor via Tailscale (serve na porta 28471). Ver `docs/OPERACAO_TAILSCALE.md`.
...
├── docs/
│   └── OPERACAO_TAILSCALE.md      # como operar a API via Tailscale (servidores/scripts)
├── prisma/
...
│   ├── config.py                  # settings (env, DATABASE_URL, API_PORT)
│   ├── auth.py                    # token validation
│   ├── db.py                      # Prisma Client Python ou asyncpg
│   ├── registry.py                # Project Registry (load from Postgres, resolve)
│   ├── ingest/
│   │   ├── __init__.py
│   │   ├── chunking.py
│   │   ├── embeddings.py
│   │   └── indexer.py             # build/update index per project
│   ├── rag/
│   │   ├── __init__.py
│   │   ├── retrieve.py            # get chunks by query
│   │   └── prompt.py              # build prompt with chunks
│   ├── jobs/
│   │   ├── __init__.py
│   │   ├── queue.py               # leitura/escrita fila no Postgres + worker limit
│   │   ├── store.py               # CRUD jobs no PostgreSQL
│   │   └── worker.py              # process one job (RAG + Ollama)
│   ├── api/
│   │   ├── __init__.py
│   │   ├── ingest.py              # /ingest
│   │   ├── ask.py                 # /ask, /status, /result
│   │   ├── health.py              # /health, /metrics
│   │   └── router_auto.py         # optional router por temáticas
│   └── models.py                  # Pydantic request/response
├── data/                          # não versionado
│   └── <project_id>/
│       ├── index/                 # vector index files
│       └── raw/                   # optional copy of ingested paths
├── projects/                      # config por projeto (opcional)
│   └── <project_id>/
│       └── config.json
├── logs/                          # não versionado
├── scripts/
...
5. **Habilitar Tailscale e expor a porta 28471**
   ...
   ```
   Detalhes para servidores e scripts: ver `docs/OPERACAO_TAILSCALE.md`.
