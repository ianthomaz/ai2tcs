...
Se precisares de um nó intermédio (ex.: VM sem TCP estável até o **llm_server**), podes subir nginx na tailnet que escuta numa porta (ex. 28472) e faz `proxy_pass` para o **llm_server** `:28471`. Modelo genérico: [`docs/refs/nginx/itcsVM2.conf`](./nginx/itcsVM2.conf) (adaptar host, IP e paths à mão). O cliente usa então `http://<IP-tailnet-do-proxy>:<porta>`. Detalhes reais: `local-only/`.
