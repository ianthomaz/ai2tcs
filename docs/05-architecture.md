...
Cenários típicos: API só em **127.0.0.1**, exposição na **tailnet** (Tailscale Serve), ou **proxy/nginx** / **túnel SSH** quando a topologia não permite TCP estável. Documentação de operação: [refs/operacao-tailscale.md](./refs/operacao-tailscale.md); exemplos de config: [refs/nginx/](./refs/nginx/).
